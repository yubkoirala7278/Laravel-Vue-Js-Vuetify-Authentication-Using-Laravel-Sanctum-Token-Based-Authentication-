...
const newColor = ref({ slug: null, name: '', status: 'active' });
...
const loadItems = debounce(async (options) => {
    ...
    const sort = options.sortBy.length ? options.sortBy[0] : { key: 'created_at', order: 'desc' };
    sortBy.value = [sort];
...
const resetNewColor = () => ({ slug: null, name: '', status: 'active' });

const openAddModal = () => {
    newColor.value = resetNewColor();
    colorStore.resetErrors();
    serverErrors.value = {};
...
const openEditModal = async (slug) => {
    try {
        await colorStore.fetchColor(slug);
        newColor.value = {
            slug,
            name: colorStore.currentColor.name,
            status: colorStore.currentColor.status,
        };
        colorStore.resetErrors();
        serverErrors.value = {};
...
const submitColor = async () => {
    ...
    if (!valid) return;
    try {
        await colorStore.storeColor(newColor.value);
        showAddModal.value = false;
    } catch (error) {
        serverErrors.value = colorStore.colorErrors;
    }
};
...
const submitEditColor = async () => {
    ...
    if (!valid) return;
    try {
        await colorStore.updateColor(newColor.value.slug, newColor.value);
        showEditModal.value = false;
    } catch (error) {
        serverErrors.value = colorStore.colorErrors;
    }
...
// === CRUD Operations ===
const editColor = slug => openEditModal(slug);

const deleteColor = async (slug) => {
    if (!confirm('Are you sure you want to delete this color?')) return;
    try {
        await colorStore.deleteColor(slug);
    } catch (error) {
...
<template>
    <v-container class="py-6">
        ...
        <v-card class="pa-4 mb-4 rounded-lg elevation-2">
            <v-row justify="space-between">
                <v-col cols="12" sm="6" md="4">
                    <v-text-field v-model="search" label="Search Colors" prepend-inner-icon="mdi-magnify"
                ...
                <v-col cols="12" sm="auto">
                    <v-btn color="teal-darken-2" class="rounded-lg px-4" @click="openAddModal">
                        <v-icon left>mdi-plus</v-icon> Add Color
                    </v-btn>
                </v-col>
            </v-row>
        </v-card>
        ...
        <v-card class="rounded-lg elevation-2">
            <v-data-table-server v-model:items-per-page="itemsPerPage" :headers="headers" :items="colorStore.colors"
                :items-length="colorStore.pagination.total || 0" :loading="colorStore.loading" item-value="slug"
                v-model:sort-by="sortBy" :items-per-page-options="[
                    { value: 5, title: '5' },
                    { value: 10, title: '10' },
                    { value: 20, title: '20' },
                    { value: 50, title: '50' },
                ...
                <template v-slot:item.sn="{ index }">
                    {{ (colorStore.pagination.current_page - 1) * itemsPerPage + index + 1 }}
                </template>
                <!-- Status -->
                ...
                <template v-slot:item.actions="{ item }">
                    <v-btn icon color="warning" size="small" @click="editColor(item.slug)">
                        <v-icon>mdi-pencil</v-icon>
                    </v-btn>
                    <v-btn icon color="red-darken-2" size="small" class="ms-2" @click="deleteColor(item.slug)">
        ...
        <v-dialog v-model="showAddModal" max-width="500" persistent transition="dialog-bottom-transition">
            <v-card rounded="xl" elevation="12">
                <v-card-title class="py-4 px-6" style="background: linear-gradient(45deg, #00695c, #00897b)">
                    <v-row align="center">
                        <v-col>
                            <span class="text-h6 font-weight-medium text-white">Add New Color</span>
                        </v-col>
                        <v-col cols="auto">
                ...
                <v-card-text class="pa-6">
                    <v-form ref="addForm" @submit.prevent="submitColor" class="d-flex flex-column gap-4">
                        <v-text-field v-model="newColor.name" label="Color Name" prepend-inner-icon="mdi-palette"
                        ...
                        <v-select v-model="newColor.status" :items="[
                            { title: 'Active', value: 'active' },
                            { title: 'Inactive', value: 'inactive' }
                        ...
                        <v-btn type="submit" color="teal-darken-2" variant="flat" size="large" block
                            :loading="colorStore.loading" class="mt-4 rounded-lg" elevation="2">
                            Save Color
                        </v-btn>
                    </v-form>
                </v-card-text>
        ...
        <v-dialog v-model="showEditModal" max-width="500" persistent transition="dialog-bottom-transition">
            <v-card rounded="xl" elevation="12">
                <v-card-title class="py-4 px-6" style="background: linear-gradient(45deg, #00695c, #00897b)">
                    <v-row align="center">
                        <v-col>
                            <span class="text-h6 font-weight-medium text-white">Edit Color</span>
                        </v-col>
                        <v-col cols="auto">
                ...
                <v-card-text class="pa-6">
                    <v-form ref="addForm" @submit.prevent="submitEditColor" class="d-flex flex-column gap-4">
                        <v-text-field v-model="newColor.name" label="Color Name" prepend-inner-icon="mdi-palette"
                            density="comfortable" variant="outlined" color="teal-darken-2" class="rounded-lg mb-4"
                            :rules="rules.name" required :error-messages="serverErrors.name" />
                        <v-select v-model="newColor.status" :items="[
